	...
	public void onClose(Session session) {
		...
		try {
			...
			for (int j = 0, jsize = userList.size(); j < jsize; j++) {
				if (!userList.get(j)[2].equals(userThatLeftRoom))
					continue;
		...
		try {
			if (json.has("cmd")) {
				...
				case "ping":
					...
					break;
				case "message":
					jsonString = "{\"message\":\"" + json.getString("body") + "\",\"username\":\""
							+ json.getString("username") + "\",\"sessionIdMessage\":\"" + json.getString("sessionId")
							+ "\"}";
					userId = json.getString("sessionId");
					room = json.getString("room");
					broadcast = true;
				...
				case "onlineUsers":
					...
					break;
				case "addUser":
					userList.add(new String[] { json.getString("sessionId"), json.getString("username"),
							json.getString("room") });
				...
				case "notifyOfNewUser":
					...
					break;
				case "messageUser":
					jsonString = "{\"messageUser\":\"" + json.getString("pm") + "\",\"from\":\""
							+ json.getString("from") + "\"}";
			...
			if (jsonString != null) {
				if (broadcast) {
					...
					for (int j = 0, jsize = userList.size(); j < jsize; j++) {
						if (!userList.get(j)[2].equals(room))
							continue; // not in this room
						if (json.getString("cmd").equals("notifyOfNewUser") && userList.get(j)[0].equals(userId))
							continue; // don't notify myself that I joined
						sessionList.get(j).getBasicRemote().sendText(jsonString);
					}

				} else {
					for (int i = 0, size = sessionList.size(); i < size; i++) {
						if (!sessionList.get(i).getId().equals(userId))
							continue;
